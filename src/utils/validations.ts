...
export const email = (value: string): ValidationInterface => {
  ...
  else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,6}$/i.test(value)) error = 'invalid-format';

  return { ok: !error, error };
};

export const password = (value: string, length: number = 8): ValidationInterface => {
  let error: ErrorType;
...
export const confirmPassword = (value: string, compareValue: string): ValidationInterface => {
  let error: ErrorType;
...
export const alphabet = (value: string): ValidationInterface => {
  ...
  if (!value) error = 'required';
  else if (!/^[A-Za-zñÑáÁéÉíÍóÓúÚöÖüÜ ]+$/.test(value)) error = 'invalid-format';
...
export const float = (value: string): ValidationInterface => {
  ...
  if (!value) error = 'required';
  else if (!/^[+-]?((\.\d+)|(\d+(\.\d+)?))$/.test(value)) error = 'invalid-format';
...
export const cardNumber = (value: string): ValidationInterface => {
  ...
  if (!value) error = 'required';
  else if (!/^[0-9]+$/.test(aux) || !(aux.length === 16 || aux.length === 15)) error = 'invalid-format';
...
export const rfc = (value: string): ValidationInterface => {
  let error: ErrorType;

  if (!value) error = 'required';
  else if (!/^([A-ZÑ&]{3,4}) ?(?:- ?)?(\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])) ?(?:- ?)?([A-Z\d]{2})([A\d])$/.test(value)) {
    error = 'invalid-format';
  }
